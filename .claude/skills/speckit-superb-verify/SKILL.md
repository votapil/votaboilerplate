...
1. **Vendored copy (authoritative in this repo):** Read
   `.specify/extensions/superb/skills/verification-before-completion/SKILL.md`.
   This is the ONLY location. Do not look in `.agent/`, `.agents/` or `.claude/skills/` —
   a second copy is how these documents silently drift apart.
2. **Remote fetch:** Only if the file above is genuinely missing, fetch
   `https://raw.githubusercontent.com/obra/superpowers/main/skills/verification-before-completion/SKILL.md`
   and tell the user the vendored copy is gone — that is a repository defect, not a normal path.
