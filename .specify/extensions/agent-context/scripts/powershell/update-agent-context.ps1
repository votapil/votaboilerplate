...
# TEMPLATE DEVIATION FROM UPSTREAM — see the bash counterpart's header for the reasoning.
# No feature path is discovered or written: the managed block carries a stable pointer so it
# can never go stale. $PlanPath is accepted for CLI compatibility and intentionally ignored.
$null = $PlanPath
...
$lines = @($MarkerStart,
           'Spec-driven features live in `specs/<NNN>-<slug>/`. When the current branch is a',
           'feature branch, read the `spec.md` in that feature directory (and `plan.md` /',
           '`tasks.md` if present) before touching its code. Otherwise ignore this block: the',
           'rules in this file are the whole context. Which lane a change belongs to is decided',
           'in `.specify/when-to-use-speckit.md`.')
$lines += $MarkerEnd
